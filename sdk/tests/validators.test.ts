...
describe("validators", () => {
  describe("matchToolCalls", () => {
    it("should match exact order and content", () => {
      expect(matchToolCalls(["add", "multiply"], ["add", "multiply"])).toBe(true);
    });

    it("should fail on wrong order", () => {
      expect(matchToolCalls(["add", "multiply"], ["multiply", "add"])).toBe(false);
    });
  ...
  describe("matchToolCallsSubset", () => {
    ...
    it("should match regardless of order", () => {
      expect(matchToolCallsSubset(["multiply", "add"], ["add", "multiply"])).toBe(true);
    });

    it("should match exact set", () => {
      expect(matchToolCallsSubset(["add", "multiply"], ["add", "multiply"])).toBe(true);
    });

    it("should fail when expected tool is missing", () => {
      expect(matchToolCallsSubset(["add", "subtract"], ["add", "multiply"])).toBe(false);
    });
    ...
    it("should not require multiple occurrences", () => {
      ...
      expect(matchToolCallsSubset(["add", "add"], ["add", "multiply"])).toBe(true);
    });
  });

  describe("matchAnyToolCall", () => {
    it("should match when at least one expected is present", () => {
      expect(matchAnyToolCall(["add", "subtract"], ["multiply", "add"])).toBe(true);
    });

    it("should fail when none of expected are present", () => {
      expect(matchAnyToolCall(["add", "subtract"], ["multiply", "divide"])).toBe(false);
    });
  ...
  describe("matchToolCallCount", () => {
    it("should match exact count", () => {
      expect(matchToolCallCount("add", ["add", "add", "multiply"], 2)).toBe(true);
    });
    ...
    it("should count single occurrence", () => {
      expect(matchToolCallCount("add", ["add", "multiply", "divide"], 1)).toBe(true);
    });
  });
