...
graph TB
    subgraph DevelopmentMode["Development Mode"]
        DevClient["Vite Dev Server<br/>Port 8080"]
        DevServer["Hono Server<br/>Port 3000"]
        DevClient -.HTTP.-> DevServer
    end

    subgraph WebProduction["Web Production"]
        WebServer["Hono Server<br/>Port 3001"]
    ...
    subgraph ElectronApp["Electron App"]
        ...
        EmbeddedHono["Embedded Hono<br/>127.0.0.1 Port 3000"]
    ...
    subgraph Docker["Docker"]
        ...
        DockerHono["Hono Server<br/>Port 3001"]
...
flowchart LR
    ...
    SDK --> Client["Build Client<br/>Vite build to dist/client"]
    Client --> Server["Build Server<br/>tsx build to dist/server"]
...
flowchart TD
    ...
    CheckNodeEnv -->|development| WebDev["Web Development<br/>- API only mode<br/>- CORS enabled for port 8080<br/>- Separate Vite server"]
...
sequenceDiagram
    participant User
    participant Renderer as Renderer Process (React App)
    ...
    AuthProvider->>Browser: Redirect to mcpjam callback with code
    Browser->>Main: Deep link captured

    Note over Main: Parse URL, extract code and state

    Main->>Main: Build callback URL with params
    Main->>Renderer: Load callback URL via loadURL
    Main->>Renderer: Send IPC oauth-callback event

    Note over Renderer: useElectronOAuth hook listens

    Renderer->>Renderer: Extract code and state from params
    Renderer->>Renderer: Navigate to callback route
    Renderer->>AuthProvider: Complete OAuth flow via AuthKit
...
flowchart TD
    ...
    CheckMode -->|Electron Packaged| ElectronProd["Load from process.resourcesPath<br/>.env.production"]
    CheckMode -->|Electron Dev| ElectronDev["Load from app.getAppPath<br/>.env.development"]
    CheckMode -->|Web/Docker| WebMode["Load from packageRoot<br/>.env.production or .env.development"]

    ElectronProd --> Validate{"CONVEX_HTTP_URL<br/>exists?"}
    ElectronDev --> Validate
    WebMode --> Validate

    Validate -->|No| Error["Throw Error<br/>Show env path and debug info"]
...
flowchart TD
    ...
    Frontend -->|Yes| ReactComp["Create or Modify<br/>React Components<br/>in client/src"]
    Frontend -->|No| Backend
    ...
    SDK --> Test["Write Tests<br/>vitest and playwright"]
