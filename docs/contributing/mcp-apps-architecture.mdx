...
<Note>
  MCPJam Inspector also supports [OpenAI Apps SDK](./openai-sdk-architecture) and
  [MCP-UI](./playground-architecture#mcp-ui-integration) for custom UI rendering.
  MCP Apps takes priority when a tool has `ui/resourceUri` metadata.
</Note>
